...
function renderArticleSection(section, sectionName) {
    return function (req, res, next) {
        articleManager.getAllArticles({limit: 10000, section: section}, (err, articles) => {
            if (err) {
                return next(err);
            }
...
router.route('/article/:slug')
    .get(function (req, res, next) {
        ...
        articleManager.getArticleBySlug(slug, (err, article) => {
            ...
            let loggedIn = req.session.username == process.env.USERNAME && req.session.password == process.env.PASSWORD;
    ...
    .delete(function (req, res, next) {
        if (!(req.session.username == process.env.USERNAME && req.session.password == process.env.PASSWORD)) {
            return next(new Error('unauthorized'));
        }
        let slug = req.params.slug;
        articleManager.deleteArticleBySlug(slug, (err) => {
            if (err) {
                return next(err);
            }
